...
scores_NN_cover2=np.mean(cross_val_score(mlpfit_cover4,X_train,Y_train,cv=3,scoring='roc_auc'))

mlp_probas =  mlp.predict_proba(X_test)
y_pred = [1 if x >= 0.3 else 0 for x in mlp_probas[:, 1]]
cm_mlp=confusion_matrix(Y_test,y_pred) #confusion matrix 
